...
check("20 article topics",rules.ARTICLE_TOPICS.length===20&&new Set(rules.ARTICLE_TOPICS).size===20);
check("8 image types",rules.IMAGE_TYPES.length===8&&new Set(rules.IMAGE_TYPES).size===8);
check("5 comment questions",rules.COMMENT_QUESTIONS.length===5&&new Set(rules.COMMENT_QUESTIONS).size===5);
...
check("Chinese operation labels",["FB信任貼文","品牌與寫作語氣","開場方式","敘事結構","應用程式金鑰","內容安全檢查","重新生成不同內容指紋","觸及人數","成效與質性備註"].every(x=>text.includes(x))&&!pageText.includes("臉書信任文章"));
check("article choice selects",["topic","visualType","commentQuestion"].every(id=>pageText.includes(`<select id="${id}"></select>`))&&text.includes('ccps-rules.js?v=20260826-form-options-v1')&&text.includes('fill("topic",ARTICLE_TOPICS)')&&text.includes('fill("visualType",IMAGE_TYPES)')&&text.includes('fill("commentQuestion",COMMENT_QUESTIONS)'));
